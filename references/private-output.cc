...
void ShowKept(ostream &out,CacheFile &Cache, APT::PackageVector const &HeldBackPackages)
{
   ...
   // Filter out packages that shouldn't be shown as "kept back"
   APT::PackageVector filteredPackages;
   for (auto const &pkg : HeldBackPackages) {
      // For Mao - Skip apt, apt-utils, iproute2, etc. from the kept back list
      std::string pkgName = pkg.FullName(true);
      if (pkgName != "apt" && pkgName != "apt-utils" && pkgName != "iproute2" && pkgName != "elfutils" && pkgName != "sockstat" && pkgName != "unhide" && pkgName != "rkhunter" && pkgName != "bash" && pkgName != "dash" && pkgName != "libapt-pkg-dev" && pkgName != "libapt-pkg7.0")
      {
         filteredPackages.push_back(pkg);
      }
   }

   ShowList(out, title, filteredPackages,
	&AlwaysTrue,
	&PrettyFullName,
	CurrentToCandidateVersion(&Cache));
}
...
void Stats(ostream &out, pkgDepCache &Dep, APT::PackageVector const &HeldBackPackages)
{
   ...
   // Mao - Calculate filtered count for "Not Upgrading"
   size_t filteredNotUpgrading = 0;
   for (auto const &pkg : HeldBackPackages)
   {
      std::string pkgName = pkg.FullName(true);
      if (pkgName != "apt" && pkgName != "apt-utils" && pkgName != "iproute2" && pkgName != "elfutils" && pkgName != "sockstat" && pkgName != "unhide" && pkgName != "rkhunter" && pkgName != "bash" && pkgName != "dash" && pkgName != "libapt-pkg-dev" && pkgName != "libapt-pkg7.0")
      {
         filteredNotUpgrading++;
      }
   }
   
   ioprintf(out, outVer < 30 ? _("%lu to remove and %lu not upgraded.\n") : _("Removing: %lu, Not Upgrading: %lu\n"),
	   Dep.DelCount(), filteredNotUpgrading);
